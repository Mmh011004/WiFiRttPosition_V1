...
import com.example.wifirttposition.entity.LatLng;
...
public class FMCoordTransformer {
    ...
    /**
     *
     * @param PreviousCoord 上一个坐标
     * @param Angle 当前角度
     * @param StepLength 步长
     * @return Fengmap坐标
     */
    public FMMapCoord nextStepCoord(FMMapCoord PreviousCoord, float Angle, float StepLength){
        FMMapCoord nextCoord = new FMMapCoord();
        double radians = Math.toRadians(Angle);
        nextCoord.x = PreviousCoord.x + ((float) Math.cos(radians) * StepLength);
        nextCoord.y = PreviousCoord.y + ((float) Math.sin(radians) * StepLength);
        return nextCoord;
    }
